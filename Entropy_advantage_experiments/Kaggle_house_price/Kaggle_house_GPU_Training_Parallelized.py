...
shuffled_df = pd.read_csv('../General_training/Kaggle_house/train_shuffled.csv')
...
# Set seed for reproducibility, for parallel, cannot set a constant seed
#set_seed(123)
...
net = nn.Sequential(nn.Linear(331, width), nn.ReLU(), nn.Linear(width, 1))
...
def Train_WLMD(net, X_train_WL, y_train_WL, entropy, entropyBias, histogram, numepochs, bin_num, bin_width, start_epoch, time_step=None, velocity=None):
    ...
    for epoch in range(start_epoch, start_epoch+numepochs):
        ...
        if epoch % 1000 == 0:
            E_traj.append(kineticEnergy.item())
